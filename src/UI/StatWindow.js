    ...
    create() {
        ...
        let html = `<div style=" width: 270px; height: 630px; background-color: rgb(30,30,30); border-radius: 10px; padding: 10px;"> 
                            ...
                            <details style="background-color: rgb(20,20,20); border-radius: 10px;" open>
                                <summary style="font: 24px kreon; color: darkGray;user-select: none; background-color: rgb(45,45,45); border-bottom: 2px ridge DimGray; "> 
                                    <i class="fa fa-chart-simple" style="width: 10%; height:10%;"></i> Stats 
                                </summary>
                                <div id="combo-count" style="font: 24px kreon; padding-left: 5%; color: DarkGray; user-select:none; data-combos="0"; > Combos:
                                    <span style="color:tan;"> </span>
                                </div>
                                <div id="swap-count" style="font: 24px kreon; padding-left: 5%; color: DarkGray; user-select:none; data-swaps="0"; > Swaps:
                                    <span style="color:tan;"> </span>
                                </div>
                            </details>
                        ...
                        <details style="" open>
                            ...
                            <div id="combo-paths" style="height: 500px; background-color: rgb(20,20,20); border-radius: 10px; overflow-y: scroll;" >
                            </div>
                        </details>
                    </div>`
        ...
        for (let i = 0; i < 20; i++) {
            ...
            if(result.solutionList[i].comboList.length > 1){
                ...
                for (let combo of result.solutionList[i].comboList) {
                    ...
                    h += ` <img src="assets/UI/${color}.svg" style = "width: 10%; height: 10%; padding: 0px 5px;"> x${combo.number}`;
                }
                let html = ` 
                    <div class= "orbs-matched" data-path="${i}"> ${h} </div>
               `;
                //pathStats.push({comboCount: result.solutionList[i].comboList.length, swapCount: result.solutionList[i].path.length});
                document.getElementById("combo-paths").innerHTML += html;

            }
        }

        document.querySelectorAll('.orbs-matched').forEach(item => {
            item.addEventListener('click', () => {
                let prev = document.querySelector(".path-select");
                if (prev != null) {
                    prev.classList.remove("path-select");
                }
                item.classList.add("path-select");
                //swapEle.textContent = pathStats;
                this.scene.pathManager.setPath(item.dataset.path);
